    ...
    if (indexPath.section == 0) {
        switch (indexPath.row) {
            case 0:
                ...
                NSRange range = [self.detailModel.inputDate rangeOfString:@"."];
                if (range.location != NSNotFound) {
                    cell.contextLabel.text  = [self.detailModel.inputDate substringToIndex:range.location];
                }else{
            ...
            case 2:
            {
                ...
                if ([self.detailModel.messageType isEqualToString:@"voiceAlarm"]) {
                    //语音
                    LeftTitleButton *button = [LeftTitleButton buttonWithType:UIButtonTypeCustom];
                    [button setFrame:CGRectMake(Screen_Width - 12 - 100, 10, 100, 30)];
                    button.shouldUseLeftTitle = YES;
                    ...
                    NSString *audioPath = self.detailModel.message;
                    NSURL *audioURL = [NSURL URLWithString:audioPath];
                    [audioURL timeIntervalForMediaWithCompeletion:^(NSTimeInterval mediaDuation) {
                        [button setTitle:[NSString stringWithFormat:@"%.0lf\"",mediaDuation] forState:UIControlStateNormal];
                    }];
                }
                if ([self.detailModel.messageType isEqualToString:@"imgAlarm"]) {
                    ...
                    for (int i = 0; i < ([[arr lastObject] isEqualToString:@""]?arr.count -1:arr.count);i++ ) {
    ...
    if (indexPath.section == 1) {
        switch (indexPath.row) {
            case 0:
                ...
                cell.contextLabel.text = self.detailModel.policeName;
                break;
            case 1:
            {
...
- (void)requestAlarmHistoryDetail
{
    ...
    [APIManager alarmHistoryDetailWithParams:params success:^(NSDictionary *dict) {
        ...
        if (dict != nil) {
            if (dict[@"Data"] != nil) {
                if (dict[@"Data"][@"model"] != nil && [dict[@"Data"][@"model"] count] > 0) {
